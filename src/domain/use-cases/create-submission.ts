import type { FormSchema } from '../entities/schema/form-schema';
import type { FormSubmission } from '../entities/submission/form-submission';
import type { FormValues } from '../entities/submission/field-values';
import type { ISubmissionRepository } from '../repositories/submission-repository';
import { buildFormSubmission } from '../services/submission-result-builder';
import { generateSubmissionId } from '../../shared/utils/id';
import type { Result } from '../../shared/types/result';
import { err } from '../../shared/types/result';

export interface CreateSubmissionInput {
  schema: FormSchema;
  values: FormValues;
  visibleFieldIds: ReadonlySet<string>;
}

export class CreateSubmissionUseCase {
  constructor(
    private readonly submissionRepository: ISubmissionRepository,
  ) {}

  async execute(input: CreateSubmissionInput): Promise<Result<FormSubmission>> {
    const { schema, values, visibleFieldIds } = input;

    if (visibleFieldIds.size === 0) {
      return err('No visible fields to submit');
    }

    const submission = buildFormSubmission(
      schema,
      values,
      visibleFieldIds,
      generateSubmissionId(),
    );

    return this.submissionRepository.create(submission);
  }
}
